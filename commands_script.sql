...
/*COPY FILES INTO TEMPORARY TABLES FROM @project */

COPY INTO covid_tmp FROM @project/yelp_academic_dataset_covid_features.json file_format=(type=JSON);

COPY INTO tip_tmp FROM @project/yelp_academic_dataset_tip.json file_format=(type=JSON);

COPY INTO user_tmp FROM @project/yelp_academic_dataset_user.json file_format=(type=JSON);

COPY INTO review_tmp FROM @project/yelp_academic_dataset_review.json file_format=(type=JSON);

COPY INTO business_tmp FROM @project/yelp_academic_dataset_business.json file_format=(type=JSON);

COPY INTO checkin_tmp FROM @project/yelp_academic_dataset_checkin.json file_format=(type=JSON);

COPY INTO precipitation_tmp FROM @project/precipitation-inch.csv file_format=(type=csv field_delimiter=',' skip_header=1);

COPY INTO temperature_tmp FROM @project/temperature-degreeF.csv file_format=(type=csv field_delimiter=',' skip_header=1);

UPDATE PRECIPITATION_TMP SET precipitation_tmp = 8888 WHERE precipitation_tmp = 'T';



/* TABLE CREATION IN ODS AND TRANSFORM/TRANSFER OF DATA FROM STAGING TO ODS*/
...
CAST(precipitation_normal AS FLOAT) FROM "PROJECTDB"."STAGING".precipitation_tmp;
...
CAST(normal_max_tmp AS FLOAT) FROM "PROJECTDB"."STAGING".temperature_tmp;
...
SELECT parse_json($1):business_id,
	...
	FROM "PROJECTDB"."STAGING".tip_tmp;
...
SELECT parse_json($1):business_id,
			...
			parse_json($1):hours
	FROM "PROJECTDB"."STAGING".business_tmp;
...
SELECT parse_json($1):average_stars,
	...
	FROM "PROJECTDB"."STAGING".user_tmp;
...
SELECT parse_json($1):business_id, 
	...
	parse_json($1):text, 
	parse_json($1):useful, 
	parse_json($1):user_id FROM "PROJECTDB"."STAGING".review_tmp;
...
SELECT parse_json($1):business_id,
		parse_json($1):date 
		FROM "PROJECTDB"."STAGING".checkin_tmp;
...
SELECT 
	...
	parse_json($1):"highlights" FROM "PROJECTDB"."STAGING".covid_tmp;
...
/* SQL QUERY TO CREATE A FACT TABLE - TOTAL_INFO*/	
...
CREATE TABLE covid CLONE "PROJECTDB"."ODS".covid ;

CREATE TABLE tip CLONE "PROJECTDB"."ODS".tip;

CREATE TABLE user CLONE "PROJECTDB"."ODS".user ;

CREATE TABLE review CLONE "PROJECTDB"."ODS".review;

CREATE TABLE business CLONE "PROJECTDB"."ODS".business ;

CREATE TABLE checkin CLONE "PROJECTDB"."ODS".checkin ;

CREATE TABLE precipitation CLONE "PROJECTDB"."ODS".precipitation;

CREATE TABLE temperature CLONE "PROJECTDB"."ODS".temperature;

CREATE TABLE total_info CLONE "PROJECTDB"."ODS".total_info;
